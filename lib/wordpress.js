...
/** Lightweight title-only search index for local/browser search. */
export async function getSearchIndex(first = 200) {
 const data = await fetchWP(
 `
 ...
 category: post.categories?.nodes?.[0]?.name || 'Article',
...
/** Recent posts for cards, archives and sidebar widgets */
...
/**
 * Fetch static page content from WordPress by slug.
 * How to use: create a WordPress Page with the same slug (about, contact, etc.)
 */
...
/**
 * Fetch posts by author slug from WordPress
 * Usage: getPostsByAuthor('dispa', 6)
 */
export async function getPostsByAuthor(authorSlug, first = 6) {
 const data = await fetchWP(
 `
 query PostsByAuthor($authorSlug: String!, $first: Int!) {
 posts(
 first: $first
 where: { authorName: $authorSlug, orderby: { field: MODIFIED, order: DESC } }
 ) {
 nodes {
